...
from model.network import Encoder, discriminatorCDAN, discriminatorDANN, AdversarialLayer
import numpy as np
...
from utils.utils import inv_lr_scheduler, test_target, loop_iterable
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join("snapshot" , args.dataset , args.out_dir, f'{args.source}-{args.target}')
    ...
    if args.dataset in ["office31"]:
        ...
        source_dataset_list = ImageList(args.data_dir, open(dataset_source).readlines(),
                                transform=prep.image_train(resize_size=256, crop_size=224))
        
        print(f"{len(source_dataset_list)} source samples")

        dataset_loaders["source"] = torch.utils.data.DataLoader(source_dataset_list, batch_size=batch_size['train'],
                                                            shuffle=True, num_workers=8,
                                                                drop_last=True)


        target_dataset_list = ImageList(args.data_dir, open(dataset_target).readlines(),
                                transform=prep.image_train(resize_size=256, crop_size=224))
        dataset_loaders["target"] = torch.utils.data.DataLoader(target_dataset_list, batch_size=batch_size['train'], shuffle=True,
                                                            num_workers=8, drop_last=True)
        print(f"{len(target_dataset_list)} target samples")


        target_dataset_test_list = ImageList(args.data_dir, open(dataset_test).readlines(),
                                    transform=prep.image_test(resize_size=256, crop_size=224))
        dataset_loaders["test"] = torch.utils.data.DataLoader(target_dataset_test_list, batch_size=batch_size['val'], shuffle=False,
                                                            num_workers=8)
        print(f"{len(target_dataset_test_list)} target test samples")
