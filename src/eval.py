...
if __name__ == '__main__':
    ...
    parser.add_argument('--save' , type=str, help="text file to store to")
    ...
    id_str = f'Target: {args.target}; Checkpoint: {args.checkpoint}'
    print_str = "\nCorrect Predictions: {}/{}".format(int(accuracy.sum), accuracy.count)
    print_str1 = '\ntest_acc:{:.4f}'.format(accuracy.avg)
    print(id_str + print_str + print_str1)

    with open(args.save, "w") as text_file:
        text_file.write(id_str + print_str + print_str1)